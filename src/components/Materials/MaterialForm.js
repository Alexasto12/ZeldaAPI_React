...
const MaterialForm = ({ material, onSave, onCancel }) => {
  ...
  // Efectos de cocción de Zelda Breath of the Wild
  ...
  // Para manejar la entrada de common_locations como texto separado por comas
  const [locationsInput, setLocationsInput] = useState('');
  const [previewImage, setPreviewImage] = useState('');

  useEffect(() => {
    // Generar el ID automáticamente como el máximo ID + 1
    let nextId = 1;
    if (materials && materials.length > 0) {
      nextId = Math.max(...materials.map(m => m.id_num)) + 1;
    }

    if (material) {
      setFormData({
        name: material.name || '',
        common_locations: material.common_locations || [],
        cooking_effect: material.cooking_effect || '',
        description: material.description || '',
        hearts_recovered: material.hearts_recovered || 0,
        id_num: material.id_num || nextId,
        image: material.image || ''
      });
      
      // Convertir el array de ubicaciones a texto separado por comas
      setLocationsInput((material.common_locations || []).join(', '));
      setPreviewImage(material.image || '');
    } else {
      // Resetear el formulario cuando no hay material
      setFormData({
        name: '',
        common_locations: [],
        cooking_effect: '',
  ...
  const validateField = (name, value) => {
    ...
    switch (name) {
      ...
      case 'description':
        ...
        break;
      case 'hearts_recovered':
        if (value > 9) {
          error = 'No se pueden recuperar más de 9 corazones.';
        }
        break;
  ...
  const handleSubmit = (e) => {
    ...
    // Añadir la categoría fija "material" antes de guardar
    onSave({...formData, category: 'material'});
  };

  return (
    <div className="animate-fadeIn">
      <h2 className="text-2xl font-bold text-blue-400 mb-6 pb-2 border-b border-gray-700">
        {material ? 'Editar Material' : 'Añadir Nuevo Material'}
      </h2>
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {/* Corazones recuperados */}
          <div className="form-group">
            <label htmlFor="hearts_recovered" className="block text-sm font-medium text-gray-300 mb-1">
              Corazones recuperados:
            </label>
            <div className="flex items-center">
              <input
                ...
                step="0.25"
                required
                className={`w-full bg-gray-700 text-gray-100 border ${errors.hearts_recovered ? 'border-red-500' : 'border-gray-600'} rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all duration-200`}
              />
            ...
            {errors.hearts_recovered && <p className="text-red-500 text-sm mt-1">{errors.hearts_recovered}</p>}
          </div>
          
          {/* Efecto de cocción como select */}
          <div className="form-group">
            <label htmlFor="cooking_effect" className="block text-sm font-medium text-gray-300 mb-1">Efecto de cocción:</label>
            <select
              id="cooking_effect"
              name="cooking_effect"
              value={formData.cooking_effect}
              onChange={handleChange}
              className="w-full bg-gray-700 text-gray-100 border border-gray-600 rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all duration-200"
            >
              {cookingEffects.map((effect, index) => (
                <option key={index} value={effect}>
                  {effect ? effect : "Sin efecto"}
                </option>
              ))}
            </select>
          </div>
        ...
        <div className="form-group">
          ...
          <input
            ...
            onChange={handleChange}
            className={`w-full bg-gray-700 text-gray-100 border ${errors.common_locations ? 'border-red-500' : 'border-gray-600'} rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all duración-200`}
            placeholder="Ej: Bosque de Hyrule, Montaña de la Muerte"
          />
